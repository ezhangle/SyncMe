#include "../../Headers/Dropbox/SM_SM_QDropbox.h"
...
SM_QDropbox::Error SM_QDropbox::error()
{
    return errorState;
}

QString SM_QDropbox::errorString()
{
    return errorText;
}

void SM_QDropbox::setApiUrl(QString url)
{
...
QString SM_QDropbox::apiUrl()
{
    return apiurl.toString();
}

void SM_QDropbox::setAuthMethod(OAuthMethod m)
{
    oauthMethod = m;
    prepareApiUrl();
    return;
...
SM_QDropbox::OAuthMethod SM_QDropbox::authMethod()
{
    return oauthMethod;
}

void SM_QDropbox::setApiVersion(QString apiversion)
{
    if(apiversion.compare("1.0"))
    {
        errorState = SM_QDropbox::VersionNotSupported;
        errorText  = "Only version 1.0 is supported.";
        emit errorOccured(SM_QDropbox::VersionNotSupported);
        return;
    }
...
QString SM_QDropbox::generateNonce(qint32 length)
{
    QString clng = "";
...
QString SM_QDropbox::oAuthSign(QUrl base, QString method)
{
    if(oauthMethod == SM_QDropbox::Plaintext){
#ifdef SM_QTDROPBOX_DEBUG
    ...
    QString signature = "";
    if(oauthMethod == SM_QDropbox::HMACSHA1)
        signature = hmacsha1(baseurl.toUtf8(), key.toUtf8());
    else
    {
        errorState = SM_QDropbox::UnknownAuthMethod;
...
void SM_QDropbox::prepareApiUrl()
{
    //if(oauthMethod == SM_QDropbox::Plaintext)
...
int SM_QDropbox::sendRequest(QUrl request, QString type, QByteArray postdata, QString host)
{
    ...
    requestMap[reqnr].type   = SM_DROPBOX_REQ_CONNECT;
    ...
    {
        errorState = SM_QDropbox::UnknownQueryMethod;
...
void SM_QDropbox::responseTokenRequest(QString response)
{
    parseToken(response);
...
int SM_QDropbox::responseDropboxLogin(QString response, int reqnr)
{
    Q_UNUSED(reqnr);
...
void SM_QDropbox::responseAccessToken(QString response)
{
    parseToken(response);
...
QString SM_QDropbox::signatureMethodString()
{
    QString sigmeth;
    switch(oauthMethod)
    {
    case SM_QDropbox::Plaintext:
        sigmeth = "PLAINTEXT";
        break;
    case SM_QDropbox::HMACSHA1:
        sigmeth = "HMAC-SHA1";
        break;
    default:
        errorState = SM_QDropbox::UnknownAuthMethod;
...
void SM_QDropbox::parseToken(QString response)
{
    clearError();
#ifdef SM_QTDROPBOX_DEBUG
...
void SM_QDropbox::setKey(QString key)
{
#ifdef SM_QTDROPBOX_DEBUG
    qDebug() << "appKey = " << key;
#endif
...
QString SM_QDropbox::key()
{
    return _appKey;
}

void SM_QDropbox::setSharedSecret(QString sharedSecret)
{
#ifdef SM_QTDROPBOX_DEBUG
...
QString SM_QDropbox::sharedSecret()
{
    return _appSharedSecret;
}

void SM_QDropbox::setToken(QString t)
{
    oauthToken = t;
}

QString SM_QDropbox::token()
{
    return oauthToken;
}

void SM_QDropbox::setTokenSecret(QString s)
{
#ifdef SM_QTDROPBOX_DEBUG
...
QString SM_QDropbox::tokenSecret()
{
    return oauthTokenSecret;
}

QString SM_QDropbox::appKey()
{
    return _appKey;
}

QString SM_QDropbox::appSharedSecret()
{
    return _appSharedSecret;
}

QString SM_QDropbox::apiVersion()
{
    return _version;
}
...
bool SM_QDropbox::requestAccessTokenAndWait()
{
...
SM_Dropbox_Account SM_QDropbox::requestAccountInfoAndWait()
{
    requestAccountInfo(true);
    SM_Dropbox_Account a(_tempJson.strContent(), this);
    _account = a;
    return _account;
}
...
QUrl SM_QDropbox::requestSharedLinkAndWait(QString file)
{
    requestSharedLink(file,true);
    SM_Dropbox_Json json(_tempJson.strContent());
...
SM_Dropbox_DeltaResponse SM_QDropbox::requestDeltaAndWait(QString cursor, QString path_prefix)
{
    requestDelta(cursor, path_prefix, true);
    SM_Dropbox_DeltaResponse r(_tempJson.strContent());

    return r;
}

void SM_QDropbox::startEventLoop()
{
#ifdef SM_QTDROPBOX_DEBUG
    qDebug() << "SM_QDropbox::startEventLoop()" << endl;
#endif
    if(_evLoop == NULL)
...
void SM_QDropbox::stopEventLoop()
{
#ifdef SM_QTDROPBOX_DEBUG
    qDebug() << "SM_QDropbox::stopEventLoop()" << endl;
#endif
    if(_evLoop == NULL)
        return;
#ifdef SM_QTDROPBOX_DEBUG
...
void SM_QDropbox::requestRevisions(QString file, int max, bool blocking)
{
    clearError();
    ...
    if(blocking)
    {
        requestMap[reqnr].type = SM_DROPBOX_REQ_BREVISI;
        startEventLoop();
    }
    else
        requestMap[reqnr].type = SM_DROPBOX_REQ_REVISIO;

    return;
}

QList<SM_Dropbox_FileInfo> SM_QDropbox::requestRevisionsAndWait(QString file, int max)
{
    clearError();
    requestRevisions(file, max, true);
    QList<SM_Dropbox_FileInfo> revisionList;

    if(errorState != SM_QDropbox::NoError || !_tempJson.isValid())
        return revisionList;
    ...
    for(int i=0; i<responseList.size(); ++i)
    {
        ...
        SM_Dropbox_FileInfo revision(revData);
...
SM_DROPBOX_request SM_QDropbox::requestInfo(int rqnr)
{
    SM_DROPBOX_request request{ SM_DROPBOX_REQ_INVALID, "", "", 0 };
...
void SM_QDropbox::removeRequestFromMap(int rqnr)
{
...
void SM_QDropbox::setSaveFinishedRequests(bool save)
{
    _saveFinishedRequests = save;
}

bool SM_QDropbox::saveFinishedRequests()
{
